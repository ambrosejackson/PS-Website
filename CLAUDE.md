...
5. Header structure is identical on every page: hamburger + large Private Stock logo left; `BRANDS  STORE LOCATOR  YOUR REWARDS  [login] [cart]` right in condensed uppercase; underline hover animation. **BRANDS is the current label — D-056 REVERSED D-021's rename to CATALOG. Do not rename it back.** BRANDS still swaps the hero on hover (its asset's `nav_target` has always been `BRANDS`, so label and data now agree), but it is a dropdown, not a single action: hover on desktop / tap on touch opens a panel listing the allowlisted brands (each → `/{slug}`) with a **Catalog** entry beneath the last brand that opens the Brand Book flip-book modal — the behaviour the header item itself used to carry. It lives in `components/site/BrandsNav.tsx`. Unlike STORE LOCATOR / YOUR REWARDS it is **not** `hidden md:`— it is the header's only route to the brand pages and the Brand Book on a phone. Two chrome treatments, both driving the hero hover-swap:
