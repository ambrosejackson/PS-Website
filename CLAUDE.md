...
5. Header structure is identical on every page: hamburger + large Private Stock logo left; `BRANDS  STORE LOCATOR  YOUR REWARDS  [login] [cart]` right in condensed uppercase; underline hover animation. Two chrome treatments, both driving the hero hover-swap:
   - **Every non-brand page** (landing, `/products` + product detail, `/apparel`, `/news` + posts, `/about`, `/contact`, `/rewards`, `/store-locator`, 404): a **SOLID WHITE bar that SITS ABOVE the hero** — never overlaying it — with all content black (hamburger, black badge logo `private-stock-black.png`, nav text). No per-asset light/dark theming here; the bar is always white. Hero starts flush beneath the bar, full-bleed, no gap. Match `docs/reference/lovable/01-header-hero.png`. **Do not revert this to the transparent overlay.**
   - **Brand landing pages ONLY** (`/outfitters`, `/higherself`, `/terpkings`, `/savagesquadstrains`): the transparent bar stays overlaid on a full-bleed hero flush to the top of the viewport, with per-asset light/dark theme switching (`content_heroes.theme`). Opt in via `<HeroSwitcher overlayHeader>` / `<Header variant="overlay" />`.
