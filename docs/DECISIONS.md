...
- **D-090 — Event RSVPs live on privatestock.co, not Zeffy (Ambrose, 2026-09-22).** The Kickback: Bud, Bombing & BBQ (Sun Oct 25, 2026, 1–6 PM, 2929 W Fullerton Ave, free, 21+) takes RSVPs at `/events/kickback` (URL chosen by Ambrose). One RSVP per person, no plus-ones; 21+ by checkbox with ID at the door (no DOB stored); mobile optional; marketing opt-in unchecked; RSVPs stay open through the event for walk-ups. Generic `events` table so the next event is a row, not a rebuild. PRD `claude/PRD-KICKBACK-RSVP.md` Rev 3.

- **D-091 — Integration rule rewritten: one ecosystem, two databases, API-connected (Ambrose, 2026-09-22).** Supersedes the "no connection in either direction" wording of guardrail #1. Data should flow both ways; the website still never holds a PSM database key. Website → PSM goes only through purpose-built PSM Edge Functions with a shared secret (`PSM_INGEST_TOKEN`, header `x-ps-ingest-token`): `ingest-event-rsvp` and `dispensary-directory` (`docs/psm-side/W4-event-rsvp-ingest.md`). Options rejected: a PSM service key on the website server (breach radius = all of PSM incl. payroll) and merging databases.

- **D-092 — Budtender comped plates: 50, first come; waitlist after (Ambrose, 2026-09-22).** A self-identified budtender at a licensed IL dispensary gives dispensary name + town/city and gets a free BBQ plate while plates last; #51+ join a plate waitlist that Ambrose releases manually (no auto-promote). Allocation is race-safe in `claim_rsvp()` (event row lock). Budtenders are told on the form, confirmation, ticket and every email to bring a valid IDFPR dispensary agent badge; badges are checked in person only, no badge numbers collected online. Counsel review: not needed (Ambrose).

- **D-093 — Scheduled reminders, email only (Ambrose, 2026-09-22).** Admin-editable schedule per event (absolute time or "N days from event day at HH:MM" CT), audience filters, merge-field templates, test send, per-recipient send log (unique per reminder+guest, so no duplicates), Resend batches of 100, bounce tracking via Resend webhook. Defaults: Oct 18 10:00, Oct 24 10:00, Oct 25 10:00 (all), Oct 26 11:00 (checked in). Trigger is pg_cron + pg_net in the website Supabase project every 10 min → `/api/cron/event-jobs` (Vercel Hobby crons are daily). SMS off: carriers/Twilio reject cannabis traffic (error 30940); schema keeps a `channel` column for a future cannabis-compliant provider.

- **D-094 — Event email identity (Ambrose, 2026-09-22).** From "The Kickback" <events@privatestock.co>, reply-to ambrose@privatestock.co. Reminder links use the guest's ticket token; cancel and unsubscribe need a button click on the ticket page (no GET side effects, so link scanners can't cancel tickets). Unsubscribe also sets `subscribers.consent_marketing = false` for that address.

- **D-095 — Every RSVP syncs to the PSM CRM with consent flagged; attribution via crm_qr_codes (Ambrose, 2026-09-22).** Contacts upsert by email; budtenders get `audience_type = budtender`, `role_title = Budtender` and `account_id` = the PSM account picked in the autocomplete (auto-link, D-024 in PRD); consent lands in new PSM columns `crm_contacts.marketing_opt_in(_at)` (approved migration). Source = one `crm_qr_codes` row "The Kickback 2026" + one `crm_signups` row per contact — no change to `crm_contacts.source`. Renegades of Funk get no list access.

- **D-096 — Check-in-only staff role (Ambrose, 2026-09-22).** `staff_roles` (email → `checkin`), managed at /admin/staff (creates a password account). Door staff sign in at /admin/login?next=/checkin/<slug> and can only use `/checkin/*`, which lives outside `/admin` so they never pass `isAdminEmail()`. Steps are one-way and timestamped with the staff email: check in → badge verified → plate given (plate requires badge; DB check constraint too).
